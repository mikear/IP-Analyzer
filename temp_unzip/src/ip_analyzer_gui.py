...
# --- Clase ManualAyuda (Carga desde archivo) ---
class ManualAyuda(Toplevel):
    """Ventana modal que muestra el manual de usuario desde un archivo."""
    _base_path = Path(sys._MEIPASS) if getattr(sys, 'frozen', False) else Path(__file__).parent.parent

    def __init__(self, parent, content_type: str = "manual"):
        super().__init__(parent)
        self.content_type = content_type
        if self.content_type == "about":
            self.title("Acerca de Analizador IP/ISP")
            self.file_to_load = self._base_path / "docs" / "ABOUT.html"
        else:
            self.title("Información")
            self.file_to_load = None # Or handle error
        ...
        help_text_widget = tk.Text(text_frame, wrap=tk.WORD, state='normal', padx=15, pady=15, font=("Segoe UI", 10), bd=0, relief=tk.FLAT, background=self.cget('background'))
        vsb = ttk.Scrollbar(text_frame, orient="vertical", command=help_text_widget.yview)
        help_text_widget.configure(yscrollcommand=vsb.set)
        help_text_widget.grid(row=0, column=0, sticky='nsew')
        vsb.grid(row=0, column=1, sticky='ns')

        try:
            if self.file_to_load and self.file_to_load.is_file():
                content = self.file_to_load.read_text(encoding='utf-8')
                # Llama al renderizador de la ventana principal
                parent._render_html_to_text_widget(help_text_widget, content)
            else:
                help_text_widget.config(state='normal')
                help_text_widget.insert(tk.END, f"Error: No se encontró el archivo de contenido:\n{self.file_to_load}")
                help_text_widget.config(state='disabled')
        except Exception as e:
            msg = f"Error al leer o renderizar contenido:\n{self.file_to_load}\nDetalle: {e}"
            help_text_widget.config(state='normal')
            help_text_widget.delete('1.0', tk.END)
            help_text_widget.insert(tk.END, msg)
            help_text_widget.config(state='disabled')
            logging.getLogger(__name__).error(f"Error en ManualAyuda: {msg}", exc_info=True)

        button_frame = ttk.Frame(main_frame)
        button_frame.grid(row=1, column=0, pady=(10,0))
        close_button = ttk.Button(button_frame, text="Cerrar", command=self.destroy)
        close_button.pack()
...
class IPAnalyzerApp(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("Analizador de IP y ISP v2.2")
    ...
    def _show_about_dialog(self) -> None:
        ...
        ManualAyuda(self, content_type="about")
    ...
    def _export_report(self) -> None:
        ...
        metadata = {"investigador": self.meta_investigator.get().strip(), "juzgado_fiscalia": self.meta_court.get().strip(), "dependencia": self.meta_dependency.get().strip(), "causa_referencia": self.meta_case_id.get().strip(), "archivo_origen": Path(self.input_file_path.get()).name, "fecha_exportacion": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "zona_horaria_solicitada_gui": self.timezone.get()}
        ...
        try:
            ...
            if export_func: export_func(filepath_obj, self.full_results, metadata)
    ...
    def _render_html_to_text_widget(self, text_widget: tk.Text, html_content: str) -> None:
        # Define fonts
        default_font = tkFont.nametofont("TkDefaultFont").actual()
        base_fam = default_font.get('family', 'Segoe UI')
        base_size = default_font.get('size', 9)

        h1_font = tkFont.Font(text_widget, (base_fam, base_size + 6, 'bold'))
        h2_font = tkFont.Font(text_widget, (base_fam, base_size + 4, 'bold'))
        h3_font = tkFont.Font(text_widget, (base_fam, base_size + 2, 'bold'))
        bold_font = tkFont.Font(text_widget, (base_fam, base_size, 'bold'))
        italic_font = tkFont.Font(text_widget, (base_fam, base_size, 'italic'))
        code_font = tkFont.Font(text_widget, (tkFont.nametofont("TkFixedFont").actual()['family'], base_size - 1))
        ...
        text_widget.tag_configure("i", font=italic_font)
        text_widget.tag_configure("a", foreground="blue", underline=True)
        text_widget.tag_configure("code", font=code_font, background="#eee", relief="flat", borderwidth=0, lmargin1=10, lmargin2=10)
        text_widget.tag_configure("li", lmargin1=20, lmargin2=20) # For list items

        text_widget.config(state='normal')
        text_widget.delete('1.0', tk.END) # Clear existing content

        # Remove HTML, HEAD, BODY tags for simpler parsing
        body_match = re.search(r"<body>(.*)</body>", html_content, re.DOTALL | re.IGNORECASE)
        if body_match:
            html_content = body_match.group(1)
        else:
            # If no body tag, assume the whole content is body
            pass

        # Split content by block-level tags for easier processing
        # This is a very simplified split, might not handle all cases
        segments = re.split(r"(<h[1-3]>.*?</h[1-3]>|<p>.*?</p>|<ul>.*?</ul>|<pre><code>.*?</code></pre>|<br>)", html_content, flags=re.DOTALL | re.IGNORECASE)
        ...
        for segment in segments:
            ...
            # Headings
            h_match = re.match(r"<h([1-3])>(.*?)</h[1-3]>", segment, re.IGNORECASE)
            if h_match:
            ...
            # Paragraphs
            p_match = re.match(r"<p>(.*?)</p>", segment, re.IGNORECASE)
            if p_match:
                content = p_match.group(1).strip()
                # Process inline elements within paragraph
                self._process_inline_html(text_widget, content)
                text_widget.insert(tk.END, "\n") # Add newline after paragraph
                continue

            # Unordered Lists
            ul_match = re.match(r"<ul>(.*?)</ul>", segment, re.DOTALL | re.IGNORECASE)
            if ul_match:
                list_items = re.findall(r"<li>(.*?)</li>", ul_match.group(1), re.DOTALL | re.IGNORECASE)
                for item_content in list_items:
                    text_widget.insert(tk.END, "• ", "li") # Bullet point
                    self._process_inline_html(text_widget, item_content)
                    text_widget.insert(tk.END, "\n")
                continue

            # Preformatted Code
            pre_code_match = re.match(r"<pre><code>(.*?)</code></pre>", segment, re.DOTALL | re.IGNORECASE)
            if pre_code_match:
                code_content = pre_code_match.group(1).strip()
            ...
            # If it's not a recognized block, treat as plain text (or process inline)
            self._process_inline_html(text_widget, segment)
            text_widget.insert(tk.END, "\n") # Add newline for unhandled blocks


        text_widget.config(state='disabled')

    def _process_inline_html(self, text_widget: tk.Text, content: str) -> None:
        # This function will process bold, italic, and links within a given content string
        # and insert them into the text_widget.
        # This is still tricky with regex and Tkinter's indexing.
        
        current_pos = 0
        while current_pos < len(content):
            # Find the next HTML tag
            tag_match = re.search(r'(<b>.*?</b>|<i>.*?</i>|<a.*?</a>|<code>.*?</code>)', content[current_pos:], re.IGNORECASE)

            if tag_match:
                # Add plain text before the tag
                if tag_match.start() > 0:
                    text_widget.insert(tk.END, content[current_pos : current_pos + tag_match.start()])

                tag_text = tag_match.group(0)
                
                # Bold
                b_match = re.match(r'<b>(.*?)</b>', tag_text, re.IGNORECASE)
                if b_match:
                    text_widget.insert(tk.END, b_match.group(1), 'b')
                # Italic
                i_match = re.match(r'<i>(.*?)</i>', tag_text, re.IGNORECASE)
                if i_match:
                    text_widget.insert(tk.END, i_match.group(1), 'i')
                # Link
                a_match = re.match(r'<a\s+href=([\'"])(.*?)\1.*?>(.*?)</a>', tag_text, re.IGNORECASE)
                if a_match:
                    url = a_match.group(2)
                    link_text = a_match.group(3)
                    text_widget.insert(tk.END, link_text, ('a', url)) # Pass URL as a tag property
                    ...
                    text_widget.tag_bind('a', '<Button-1>', lambda e, u=url: webbrowser.open_new_tab(u))
                # Code (inline)
                code_match = re.match(r'<code>(.*?)</code>', tag_text, re.IGNORECASE)
                if code_match:
                    text_widget.insert(tk.END, code_match.group(1), 'code')
                else:
                    # Unhandled tag, insert as plain text
                    text_widget.insert(tk.END, tag_text)

                current_pos += tag_match.end()
            else:
                # No more tags, insert remaining plain text
                text_widget.insert(tk.END, content[current_pos:].replace('<br>', '\n')) # Replace <br> with newline
    ...
    def _populate_chunk(self, rows_iterator: Iterator[tuple], chunk_size: int = 50):
        ...
        try:
            ...
            self.update_status(f"Mostrando {len(self.full_results)} resultados.", "green", 4000)
    ...
    def _check_progress_queue(self) -> None:
        if not self.winfo_exists() or not self.is_processing.get():
             final_val = 100 if hasattr(self,'full_results') and self.full_results else 0
             if hasattr(self,'progress_bar') and self.progress_bar.winfo_exists():
                 try: self.progress_bar['value'] = final_val
                 except tk.TclError: pass
             return
        try:
            while True: record = self.log_queue.get_nowait(); self.after_idle(self._update_log_task, record)
        except Empty: pass
        except Exception as e: logging.getLogger('GUI').error(f"Error procesando cola progreso: {e}")
        finally:
            if self.winfo_exists() and self.is_processing.get():
                self._progress_check_job = self.after(150, self._check_progress_queue)

    def _analysis_task(self, filepath: Path, backend_timezone: str, queue_handler: logging.Handler) -> None:
        self.full_results = None; analysis_successful = False
        try:
            results_backend = processing.process_ip_analysis(
                filepath, backend_timezone, self.gemini_key, self.ipinfo_token,
                self.progress_queue, queue_handler
            )
            if results_backend is not None:
                self.full_results = results_backend
                analysis_successful = True
            else:
                analysis_successful = False # Error ya fue logueado por el backend
        except Exception as e:
            analysis_successful = False; self.full_results = None
    ...
    def _start_analysis_thread(self) -> None:
        ...
        self.analysis_thread = threading.Thread(
            ...
            args=(filepath, backend_timezone, self.log_queue_handler), daemon=True
        )
